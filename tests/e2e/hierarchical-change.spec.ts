...
test_hierarchicalChangeValuePreservationData.forEach((testData) => {
  test(`test_hierarchicalChangeValuePreservation[${testData.description}]`, async ({ hierarchyPage, request }) => {
    ...
    try {
      ...
      // const highestUnitFromNewHierarchy = newHierarchy[0];
      // const highestUnitFromOldHierarchy = originalHierarchy[0];
      // await lockCompleteHierarchy(request,[highestUnitFromNewHierarchy, highestUnitFromOldHierarchy]);
      // await lockCompleteHierarchy(request,[2,3]);
